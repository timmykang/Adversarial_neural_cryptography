...
def train():
	...
	with tf.Session() as session:
		...
		f = open('Testing_Eve.txt', 'w')
 		
		for _ in range(numbers_of_eve_reset):
			train_writer = tf.summary.FileWriter('E:\\adversarial_neural_cryptography\\Testing_Eve\\adver_logs', session.graph)
			session.run(tf.variables_initializer(Eve_vars))
			print('Eve_reset')
			for eve_counter in range(max_eve_counter):
				feedDict = {plain: get_random_block(plainTextLength, batch),
							key: get_random_block(keyLength, batch)}
				for index in range(2 * iterations):
					session.run(Eve_optimizer, feed_dict=Eve_feedDict)
				Eve_error, Eve_accuracy, Eve_wrong_bits = session.run([Eve_loss, accuracy_E, Eve_bits_wrong], feed_dict=Eve_feedDict)
				Eve_accuracy_bits = Eve_accuracy * plainTextLength
				if eve_counter % 500 == 0:
					print('Eve_error:', Eve_error, 'Eve accuracy bits', Eve_accuracy_bits)
					f.write('Eve_error: {}, Eve accuracy bits: {}\n'.format(Eve_error, Eve_accuracy_bits))
			train_writer.add_summary(summary, step)	
		f.close()
