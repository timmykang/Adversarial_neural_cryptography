...
def get_block():
    tmp = get_random_block(plainTextLength, 1<<16)
    for i in range(1<<16):
        tmp1 = bin(i)[2:].zfill(16)
        for j in range(16):
            tmp[i][j] = 2*int(tmp1[j])-1
    return tmp

def get_key_block():
    tmp_num = np.random.randint(1<<16)
    tmp = get_random_block(plainTextLength, 1<<16)
    for i in range(1<<16):
        tmp1 = bin(tmp_num)[2:].zfill(16)
        for j in range(16):
            tmp[i][j] = 2*int(tmp1[j])-1
    return tmp
...
def array_to_num(output):
    tmp = ''
    output1= make_num(output)
    for i in output1:
        if i == -1:
            tmp = tmp + '0'
        else:
...
def train():
    ...
    #
    Alice_output, Bob_output, Eve_output, _ = Net._build_Network(plain, key, plainTextLength, keyLength)
    ...
    with tf.Session() as session:
        ...
        train_writer = tf.summary.FileWriter('E:\\adversarial_neural_cryptography\\Testing_Eve\\adver_logs', session.graph)
        ...
        for step in range(TRAIN_STEP):
            ...
            if Eve_error > 0.9 and Bob_error < 0.1:
                tmp_feedDict = {plain: get_block(),
                                key: get_key_block()}
                B_output = session.run([Bob_output], feed_dict = tmp_feedDict)

                for i in B_output:
                    print(array_to_num(i))
