...
TRAIN_STEP= 200000
iterations = 1
max_eve_counter = 5000
numbers_of_eve_reset = 5


def get_random_block(N, batch):
    return 2 * np.random.randint(2, size=(batch, N)) - 1

def share_random_block(N, batch, mes):
	np.random.seed(mes)
	return 2 * np.random.randint(2, size=(batch, N)) - 1
...
def train():
	...
	#
	Alice_output, Bob_output, Eve_output, Eve1_output = Net._build_Network(plain, key, plainTextLength, keyLength)
	...
	with tf.Session() as session:
		...
		train_writer = tf.summary.FileWriter('E:\\adversarial_neural_cryptography\\neural_cryptography_scheme\\adver_logs', session.graph)
		if not os.path.exists('adver_logs'):
			os.makedirs('adver_logs')
		f = open('construct_neural_network.txt', 'w')
		f1 = open('Testing_Eve1.txt', 'w')
		for step in range(TRAIN_STEP):
			...
			if Eve_error > 0.94 and Bob_error < 0.04:
				flag = 1

			if flag == 1:
				Eve1_error, Eve1_accuracy, Eve1_wrong_bits = session.run([Eve1_loss, accuracy_E1, Eve1_bits_wrong], feed_dict=Eve_feedDict)
			...
			if step % 500 == 0:
				...
				if flag == 1:
					...
					f1.write('Eve1_error: {}, Eve1 accuracy bits: {}\n'.format(Eve1_error, Eve1_accuracy_bits))
			
			if flag == 1 and step % 100 == 0:
				Eve1_feedDict = {plain: get_random_block(plainTextLength, batch // 4),
							key: get_random_block(keyLength, batch // 4)}
				for index in range(2 * iterations):
